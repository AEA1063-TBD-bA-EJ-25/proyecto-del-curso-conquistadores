...
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Niño, p.FechaNacimiento
FROM Niño n
JOIN Persona p ON n.CURP = p.CURP
WHERE MONTH(p.FechaNacimiento) IN (6, 7, 8);

--Consulta para saber que instructores tienen especialidades avanzadas
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Instructor
FROM Instructor i
JOIN Persona p ON i.CURP = p.CURP
WHERE i.CURP IN (
    SELECT CurpInstructor FROM Especialidad WHERE Nivel = 3);

--consulta para saber que niños tienen un mismo tutor
SELECT p1.Nombres + ' ' + p1.ApellidoPaterno AS Niño1,
    p2.Nombres + ' ' + p2.ApellidoPaterno AS Niño2,
    pt.Nombres + ' ' + pt.ApellidoPaterno AS TutorComún
FROM Niño n1
JOIN Niño n2 ON n1.CurpTutor = n2.CurpTutor AND n1.CURP < n2.CURP
JOIN Persona p1 ON n1.CURP = p1.CURP
JOIN Persona p2 ON n2.CURP = p2.CURP
JOIN Tutor t ON n1.CurpTutor = t.CURP
JOIN Persona pt ON t.CURP = pt.CURP;

--consulta para saber que tutores tienen una informacion incompleta
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Tutor, COALESCE(t.Telefono, 'Sin teléfono') AS Contacto
FROM Tutor t
JOIN Persona p ON t.CURP = p.CURP
WHERE t.Telefono IS NULL;

--consulta para saber que cuantas reuniones se realizan por dia en la semana 
SELECT DATENAME(WEEKDAY, Fecha) AS DiaSemana, COUNT(*) AS TotalReuniones
FROM Reunion
GROUP BY DATENAME(WEEKDAY, Fecha), DATEPART(WEEKDAY, Fecha)
ORDER BY DATEPART(WEEKDAY, Fecha);

--consulta para saber la inicial del apellido de los niños 
SELECT SUBSTRING(p.ApellidoPaterno, 1, 1) AS Inicial, COUNT(*) AS TotalNiños
FROM Niño n
JOIN Persona p ON n.CURP = p.CURP
GROUP BY SUBSTRING(p.ApellidoPaterno, 1, 1)
ORDER BY Inicial;

--Consulta para saber que especialidades son las mas comunes
SELECT Area, COUNT(*) AS TotalEspecialidades
FROM Especialidad
GROUP BY Area
ORDER BY TotalEspecialidades DESC;

--Consulta para saber que niños tienen tutores con mas de 40 años de edad
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Niño, pt.Nombres + ' ' + pt.ApellidoPaterno AS Tutor,
    DATEDIFF(YEAR, pt.FechaNacimiento, GETDATE()) AS EdadTutor
FROM Niño n
JOIN Persona p ON n.CURP = p.CURP
JOIN Tutor t ON n.CurpTutor = t.CURP
JOIN Persona pt ON t.CURP = pt.CURP
WHERE DATEDIFF(YEAR, pt.FechaNacimiento, GETDATE()) > 40;

--Consulta para saber que instructores no tienen especialidades 
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Instructor
FROM Instructor i
JOIN Persona p ON i.CURP = p.CURP
...
--consulta para tener los rangos de edades de los niños 
SELECT 
    CASE 
        WHEN DATEDIFF(YEAR, p.FechaNacimiento, GETDATE()) BETWEEN 6 AND 8 THEN '6-8 años'
        WHEN DATEDIFF(YEAR, p.FechaNacimiento, GETDATE()) BETWEEN 9 AND 11 THEN '9-11 años'
        ELSE '12+ años'
    END AS RangoEdad,
    COUNT(*) AS TotalNiños
FROM Niño n
...
--consulta para saber que niños no tienen enfermedades
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Niño
FROM Niño n
JOIN Persona p ON n.CURP = p.CURP
WHERE n.CURP NOT IN (
    SELECT CURP FROM Niño WHERE Enfermedad <> 'Ninguna'
);

--consulta para saber que clubes tienen mas unidades
SELECT TOP 5 
c.Nombre AS Club, COUNT(u.ClaveUnidad) AS TotalUnidades
FROM Club c
...
--consulta para saber las especialidades por nivel 
SELECT Area, [1] AS Básico, [2] AS Intermedio, [3] AS Avanzado
FROM (
    SELECT Area, Nivel, Nombre FROM Especialidad
) AS SourceTable
PIVOT (
    COUNT(Nombre)
    FOR Nivel IN ([1], [2], [3])
...
--consulta para saber que reuniones tienen asistencia perfecta
SELECT r.ClaveReunion, u.NombreUnidad, r.Fecha,
    CASE 
        WHEN r.Asistencia = 1 THEN 'Asistencia completa'
        ELSE 'Asistencia incompleta'
    END AS EstadoAsistencia
FROM Reunion r
JOIN Unidad u ON r.ClaveUnidadReunion = u.ClaveUnidad
WHERE r.Asistencia = 1;

--Consulta de join donde se pueden mostrar los niños con sus tutotes y un contacto de emergencia
SELECT p.Nombres + ' ' + p.ApellidoPaterno AS Niño, pt.Nombres + ' ' + pt.ApellidoPaterno AS Tutor,
    t.Telefono AS ContactoEmergencia
FROM Niño n
JOIN Persona p ON n.CURP = p.CURP
JOIN Tutor t ON n.CurpTutor = t.CURP
JOIN Persona pt ON t.CURP = pt.CURP;
